...
# q_i = [rand(-1.0:2:1) for i =1:n]

N = 10000
x_g = initgrid1d(N, box_size)
...
# scatter!(ρ_plot, x_g, q_g, label = false)
# vline!(ρ_plot, (x_g .+ (x_g[2]-x_g[1])/2)[1:end-1], label = false, lc = :black, ls = :dash)
# vline!(ρ_plot, x_i[q_i .> 0.0], label = false, lc = :red)
# vline!(ρ_plot, x_i[q_i .< 0.0], label = false, lc = :blue)
...
ϕ_fft[1] = 0.0#  4π/(1e-20) * ρ_fft_3[1]
ϕ_g = real.(ifft(ϕ_fft))
...
E_fft[1] = 0.0# -1.0im * ρ_fft_3[1] / (eps(0.0)* 2π) #* S_fft_th[1]
E_g = real.(ifft(E_fft))
